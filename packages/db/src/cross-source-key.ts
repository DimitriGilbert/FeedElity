 ...
 * byte-for-byte on the stored key (title NFC-normalized before lowercasing and
 * stripping so decomposed and precomposed accents land on one key). Both
 * implementations are pinned to the same case table by parity tests here
 * (cross-source-key.test.ts) and in packages/api (src/domain/catalog.test.ts).
 */
