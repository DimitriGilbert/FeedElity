...
4. **Scripts**: add `"db:repair": "bun run src/migrations/apply-catalog-migrations.ts"` to packages/db/package.json and `"db:repair": "turbo -F @FeedElity/db db:repair"` to the root package.json (next to the other `db:*` scripts).
